...
- **Esc+Tab**（拡張・起動中のみ）：オーバーレイを丸ごと隠す/戻す（`host.style.display`）。Esc押下中フラグ `escDown` で検出。app.toggle で必ず復帰。
- **全画面プレゼン/スライドショー対応**（拡張）：`followFullscreen` で host を全画面要素へ移動＋ `inFullscreen` 中は autoHide を止めてバー常時表示。描画/ UIクリックは `stopPropagation`（liveキャンバスの click/pointer も）で下のページに渡さない＝スライドが進まない。※Slidesがdocument/window のcaptureで入力を取る場合は完全には抑止できない可能性あり（実機要確認）。
- 矢印の矢じり既定は「始点」(`arrowHead: "start"`)。
